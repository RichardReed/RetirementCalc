...
private
  def payrole_tax(year)
    ...
    if year.is_age < 60
      income = @income_in.gross_income(year) - @income_in.lump_sum_income(year) - @ira_spending.ira_spend(year)
    else
      income = @income_in.gross_income(year) - @income_in.lump_sum_income(year) - @ira_spending.ira_spend(year) + 
		ira_to_non_ira_xfer  #Note:  IRA spending is a negative number
    end
